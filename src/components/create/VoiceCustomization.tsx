...
export function VoiceCustomization({
  ...
  const [selectedLanguage, setSelectedLanguage] = React.useState<string | null>("en-US");
  ...
  const selectedLanguageLabel = React.useMemo(() => {
    const found = allLanguagesFromLibrary.find((l) => l.code === selectedLanguage);
    return found ? found.language : "";
  }, [allLanguagesFromLibrary, selectedLanguage]);
  ...
  React.useEffect(() => {
    ...
    setIsPlaying(false);
  }, [tab, selectedLanguage, googleCloudVoice.name]);

  // --- Handlers ---

  // Play or stop preview audio
  const handlePreview = (previewUrl: string | undefined) => {
  ...
  const handleFileChangeAndClone = async (file: File | null) => {
    if (!file) return;
  ...
  const handleFileDrop = (e: React.DragEvent<HTMLLabelElement>) => {
    e.preventDefault();
    handleFileChangeAndClone(e.dataTransfer.files?.[0] || null);
  };

  const handleFileInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    handleFileChangeAndClone(e.target.files?.[0] || null);
  };
  ...
  // ElevenLabs: Preview (simulate)
  const [elevenPreviewLoading, setElevenPreviewLoading] = React.useState(false);
  const handleElevenPreview = () => {
    setElevenPreviewLoading(true);
    setTimeout(() => setElevenPreviewLoading(false), 2000); // Simulate preview
  };
      ...
      <CardContent className="space-y-6">
        <Tabs value={tab} onValueChange={(v) => setTab(v as "google" | "elevenlabs")} className="w-full">
          <TabsList className="w-full grid grid-cols-2 mb-4">
            <TabsTrigger value="google">Google Voice Library</TabsTrigger>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={elevenPreviewLoading}
                    onClick={handleElevenPreview}
                  >
                    <Volume2 className="mr-2 h-4 w-4" /> Preview
                  </Button>
                  <Button
                ...
                <div>
                  <label className="block font-medium mb-1">
                    Speed{" "}
                    <span className="ml-2 text-sm text-muted-foreground">
                      {(elevenLabsClonedVoice.speed * 100).toFixed(0)}
                    </span>
                  </label>
                  <Slider
                    min={0.7}
